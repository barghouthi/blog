...
*find a value* for $ h $
such that *for all values* of  $ x $ and $y$,
we have
...
of this formula is only over $ h $.
...
position of the first 0 from the right occuring in `x`, and 0 everywhere else.[^fn-sketch] 

[^fn-sketch]: Example borrowed from Sketch project and found through Loris D’Antoni’s notes.
